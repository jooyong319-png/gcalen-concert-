...
- **"블로그 글 필요" 신호가 있으면**: `BLOG_RESEARCHER.md`를 읽고 그 절차를 그대로 따라
  KO 아티클 1편을 작성 → 커밋.
- **"아티스트 프로필 필요" 신호가 있으면**: `ARTIST_PROFILE.md`를 읽고 그 절차를 그대로 따라
  누락된 아티스트의 이미지/소개글을 채움 → 커밋.
...
3. 실행할 때도 반드시 해당 프롬프트(`ARTIST_PROFILE.md`/`BLOG_RESEARCHER.md`)의 절차와
   절대 규칙을 그대로 따른다 — 플래너라고 그 규칙들을 건너뛰지 않는다
